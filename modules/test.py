...
class test:
    ...
    def pick(network, x_data, t_data, index):
        ...
        print(f"정답 좌표: x={str(t_x).rjust(2)},y={str(t_y).rjust(2)} ({ math.floor(scores_nomalized[t_y, t_x]*100)/100 }점, { math.floor(winning_odds[t_y, t_x]*100)/100 }%)", end=" / ")
        # print("정답 좌표: ", end="") # 답 여러개일 때
        # for t_yx, t_yx_chance in zip(t_yxs, t_yx_chances):
        #     print(f"{t_yx} ({t_yx_chance}%)", end=" / ")
        print(f"\n구한 좌표: x={str(a_x).rjust(2)},y={str(a_y).rjust(2)} ({ math.floor(scores_nomalized[a_y, a_x]*100)/100 }점, { math.floor(winning_odds[a_y, a_x]*100)/100 }%)", end=" / ")
    ...
    def random_picks(network, x_datas, t_datas, scope_l=0, scope_r=None):
        ...
        while True:
            test.pick(network, x_datas, t_datas, random.randrange(scope_l, scope_r))
            answer = input()
            if answer == "2":
                return
    
    # 맞거나 틀린 문제 골라 테스트
    def right_or_wrong_answers(network, x_datas, t_datas, wrong_idxs):
        ...
        for idx in range(len(t_datas)):
            if (idx not in wrong_idxs) == TrueR_FalseW:
                print("="*50)
                test.pick(network, x_datas, t_datas, idx)
